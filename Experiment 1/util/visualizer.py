...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
	...
	#im1 = Image.fromarray(im)
        if aspect_ratio > 1.0:
            im = Image.fromarray(im).resize(im, (h, int(w * aspect_ratio)), resample=PIL.Image.BICUBIC)
        if aspect_ratio < 1.0:
            im = Image.fromarray(im).resize(im, (int(h / aspect_ratio), w), resample=PIL.Image.BICUBIC)
