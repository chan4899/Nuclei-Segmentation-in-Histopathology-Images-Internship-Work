...
def plot_boundary_mask(image, true_masks=None, pred_masks=None, ax=None, image_id = None, id = id):
    """
    Plots provided boundaries of nuclei for a given image.
    """
    arr = np.zeros((image.shape[0], image.shape[1]))
    # arr = cv2.cvtColor(arr, cv2.COLOR_BGR2GRAY)
    out_dir = "/home/videsh/Downloads/Chandan/data science bowl data/data_science_bowl_2018/pred_im/"
    if ax is None:
        n_rows = 1
        n_cols = 1
    ...
    if pred_masks is not None:
        for i in range(pred_masks.shape[-1]):
            ...
            # print("ctr's shape ", ctr.shape)
            cv2.drawContours(arr, ctr, 0, id+1, -1)
            id = id +1
            # cv2.drawContours(arr, np.array([contours]), 0, i+1, -1)   

            for n, contour in enumerate(contours):
                ax.plot(contour[:, 1], contour[:, 0], linewidth=1, color='w')
        np.save("/home/videsh/Downloads/Chandan/data science bowl data/data_science_bowl_2018/predictions/" + image_id, arr)
        out_dir = out_dir + image_id + '.png'
        cv2.imwrite(out_dir, arr)
...
def pred_n_plot_test(model, config, test_path='../data/stage2_test_final/', save_plots=False):
    ...
    for mm, image_id in tqdm(enumerate((dataset_test.image_ids))):
        ...
        if save_plots:
            ...
            id = plot_boundary_mask(msk, true_masks=None, pred_masks=result['masks'],
                          ax=fig2.add_subplot(gs[0]), image_id = image_id, id=id)

            fig2.savefig('../masks2/' + str(image_id)+'.png', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    train_path = '/home/videsh/Downloads/Chandan/data science bowl data/stage1_train/'
    ...
    dataset_val = KaggleDataset()
    val_path = '/home/videsh/Downloads/Chandan/data science bowl data/stage1_test/'
    ...
    # for f in ls:
    pred_n_plot_test(model, config, test_path= '/home/videsh/Downloads/Chandan/data science bowl data/small_test_data/', save_plots=True)
    # Predict and plot boundaries for stage2 test
    # pred_n_plot_test(model, config, test_path='/home/videsh/Downloads/Chandan/data science bowl data/stage2_test_final/', save_plots=True)
