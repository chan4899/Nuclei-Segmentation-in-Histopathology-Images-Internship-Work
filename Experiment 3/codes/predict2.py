...
def plot_boundary_mask(image, true_masks=None, pred_masks=None, ax=None, image_id = None, id = id):
    ...
    Plots provided boundaries of nuclei for a given image.
    """
    arr = np.zeros((image.shape[0], image.shape[1]), np.float32)
    shape = (1000,1000)
    # arr = cv2.cvtColor(arr, cv2.COLOR_BGR2GRAY)
    mask = arr
    out_dir = "/home/videsh/Downloads/Chandan/data_science_bowl_data/data_science_bowl_2018/pred_im/"
    if ax is None:
        n_rows = 1
        n_cols = 1
    ...
    if pred_masks is not None:
        for i in range(pred_masks.shape[-1]):
            ...
            for j,contour in enumerate(xy):
                print ('--------------------')
                print (i)
                #print np.array(contour),np.array(contour[1])
            
                r,c = polygon(np.array(xy[j])[:,1],np.array(xy[j])[:,0],shape=shape)
                print (r,c) 
                mask[r,c]=i+1
        ...
        savemat(image_id, {'data':mask.transpose()})
        out_dir = out_dir + image_id + '.png'
        cv2.imwrite(out_dir, mask.transpose())
...
if __name__ == '__main__':
    ...
    # for f in ls:
    pred_n_plot_test(model, config, test_path= '/home/videsh/Downloads/Chandan/data_science_bowl_data/small_test_data/', save_plots=True)
